            ...
            <header class="inline-block font-semibold bg-gray-200 text-gray-700 py-5 px-6 sm:py-6 sm:px-8 sm:rounded-t-md">
                <div class="flex flex-row">
                        <p class="text-gray-200"> 
                                ...
                                <div class="ml-1 mt-1 text-gray-500">
                                    <span>  &#x3009; <a href="{{ route('car.create') }}"> Update </a></span>  
                                </div>
                        </p>
                </div>
            </header>
...
<div class="min-h-screen bg-gray-100 p-0 sm:p-12">
  <div class="mx-auto max-w-md px-6 py-12 bg-white border-0 shadow-lg sm:rounded-3xl">
    <h1 class="text-2xl font-bold mb-8 text-center text-gray-500">Update Car</h1>
    <form id="form" novalidate action="CarController@update, $car->id" method="post">
    {{ csrf_field() }}
    ...
    <input type="hidden" name="id" value="{{$car->id}}">
      ...
      <div class="relative z-0 w-full mb-5">
        ...
        >
        @if ($car->status == 0)
             <option value="0" selected>En etat</option>
             <option value="1">En panne</option>
        @elseif ($car->status == 1)
            <option value="0" >En etat</option>
             <option value="1" selected>En panne</option>
        @endif
        </select>
      ...
      <div class="relative z-0 w-full mb-5">
        <div class="relative z-0 w-full mb-5">
          <input
            type="text"
            name="withdrawal"
            value ="{{$car->withdrawal}}"
            onclick="this.setAttribute('type', 'date');"
            class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"
          />
          <label for="date" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Filing at</label>
      ...
      <div class="relative z-0 w-full mb-5">
        <div class="relative z-0 w-full mb-5">
          <input
            ...
            class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200"
          />
          <label for="date" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Withdrawal at</label>
          <span class="text-sm text-red-600   " id="error">@error('filing'){{$message}}@enderror</span>
        </div>
        ...
        <select
          ...
          class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none z-1 focus:outline-none focus:ring-0 focus:border-black border-gray-200"
        >
          <option value="zez" selected disabled hidden>{{$ownerInfo->firstname}} {{$ownerInfo->lastname}} {{$ownerInfo->call}}</option>
